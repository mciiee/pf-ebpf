...

{
  ...
  case PROTOCOL_IPV4:
    ...
    bpf_printk("Protocol: 0x%02x/0x%04x (%s/IPv4)", tproto, iproto,
               getL3ProtocolName(tproto));
    break;
  case PROTOCOL_IPV6:
    tproto = parseIPv6(ctx);
    bpf_printk("Protocol: 0x%02x/0x%04x (%s/IPv6)", tproto, iproto,
               getL3ProtocolName(tproto));
    break;
